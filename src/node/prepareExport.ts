...
export const getApkgExportData = async (
  ...
  const extraTemplateParams = String(
  ...
  for (const mediaFileId in mediaIdToClipsIds) {
    ...
    for (const id of clipIds) {
      ...
      const baseFieldOrder = fieldNames
      const extraFieldOrder = extraTemplateParams
      const imageHtml =
        flashcard.image
          ? `<img src="${basename(
      ...
      const orderedFields = [
        ...baseFieldOrder.map((fn) => roughEscape((fieldsWithHiragana as any)[fn] || '')),
        ...extraFieldOrder.map((fn) => roughEscape(((flashcard.fields as any)[fn] as string) || '')),
  ...
  const finalFieldNames = [...fieldNames, ...extraTemplateParams, 'sound', 'image']
