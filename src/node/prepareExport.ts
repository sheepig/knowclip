...
export const getApkgExportData = async (
  ...
  const tmpl = getYomitan2AnkiTemplate(state)
  const extraKeys = (tmpl?.templateParams || '')
    .split(',')
    .map((s) => s.trim())
    .filter((k) => k && !fieldNames.includes(k as any))
      ...
      clipSpecs.push({
        ...
        outputFilename,
          flashcardSpecs: {
            id: clip.id,
            fields: [
              ...[...fieldNames, ...extraKeys].map((fn) =>
                roughEscape(String((flashcard.fields as any)[fn] || ''))
              ),
  ...
  const finalFieldNames = [...fieldNames, ...extraKeys, 'sound', 'image']
