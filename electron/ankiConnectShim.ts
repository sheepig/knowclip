    ...
    switch (req.action) {
      ...
      case 'modelFieldNames': {
        ...
        try {
          ...
          fields = templateParams ? templateParams.split(',').map((s) => s.trim()).filter(Boolean) : []
        } catch {
          fields = []
        }
      ...
      case 'addNote': {
        ...
        try {
          ...
          const configuredFields = templateParams ? templateParams.split(',').map((s) => s.trim()).filter(Boolean) : []
      ...
      case 'canAddNotesWithErrorDetail': {
        try {
          ...
          const configuredFields = tmplParams
            ? tmplParams.split(',').map((s) => s.trim()).filter(Boolean)
            : []
